...
from backend.api.api_db.router import router as router_db
...
async def catch_exceptions_middleware(request: Request, call_next):
    try:
        ...
        for trace in trace_back:
            if not (
                'site-packages/starlette/' in trace[0]
                or 'site-packages/fastapi/' in trace[0]
            ):
                stack_trace.append(
        ...
        return JSONResponse(
            {'message': f'Error:{str(ex_value)}{stack_trace}'},
