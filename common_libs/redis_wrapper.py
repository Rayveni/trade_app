...
class redis_steams:
    ...
    def publish(self, topic: str, message: dict) -> bytes:
        return self.conn.xadd(topic, message)
    ...
    def __decode_message(self, messages_list: list) -> list:
        ...
        for _message in messages_list[0][1]:
            res.append(
                {
                    'topic': topic,
                    'message_id': _message[0],
                    'message': json.loads(_message[1][b'message']),
                    'header': json.loads(_message[1][b'header']),
                }
            )
        return res

    def __parse_messages(self, messages_list: list) -> list:
        res = []
        topic = messages_list[0][0]
        for _message in messages_list[0][1]:
            res.append(
                {'topic': topic, 'message_id': _message[0], 'message': _message[1]}
            )
        return res
    ...
    def consume(
        ...
        messages_list = self.conn.xreadgroup(
            consumer_group, consumer, {topic: '>'}, count=count
        )
        if len(messages_list) < 1:
            return None
        else:
            return self.__parse_messages(messages_list)
    ...
    def get_uncommited_messages(
        ...
        return self.conn.xpending_range(
            name=topic, groupname=consumer_group, min='-', max='+', count=count
        )
